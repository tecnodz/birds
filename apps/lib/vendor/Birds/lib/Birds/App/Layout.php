...
class Layout
{
    ...
    public static function find($l)
    {
        ...
        if(!file_exists($l)) {
            $cfg = \Birds\bird::app()->Birds;
            $found=false;
            if(is_array($cfg['layout-dir'])) {
                foreach($cfg['layout-dir'] as $dir) {
            ...
            } else if(file_exists($cfg['layout-dir'].'/'.$l)) {
                $l=$cfg['layout-dir'].'/'.$l;
                $found=true;
            }
            if(!$found) {
        ...
        if(substr($l, -4)=='.yml') {
            return new Layout(\Birds\Yaml::read($l));
        }
