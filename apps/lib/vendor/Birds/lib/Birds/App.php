...
class App
{
	...
	public function __construct()
	{
        ...
        $this->config = bird::recursiveReplace(
            array('$BIRD_ROOT', '$BIRD_APP_ROOT', '$BIRD_VAR', '$BIRD_VERSION'),
            array(  BIRD_ROOT,    BIRD_APP_ROOT,    BIRD_VAR,    BIRD_VERSION),
    ...
    public static function configFiles()
    {
        ...
        $cfg=array(
            BIRD_APP_ROOT.'/config/'.$name.'.yml',
            BIRD_APP_ROOT.'/config/'.$name.'@'.$server.'.yml',
            //BIRD_APP_ROOT.'/config/'.$name.'.'.$server.'.'.$domain.'.yml',
            BIRD_ROOT.'/config/bird.yml',
        );
        unset($name, $server, $domain);
        foreach($cfg as $k=>$v) {
            if(!file_exists($v)) unset($cfg[$k]);
            unset($k,$v);
        }
        return $cfg;
    }
