    ...
    if('callback' in d) {
        if(typeof d.callback == 'string') {
            if(this.id in bird.Apps && d.callback in bird.Apps[this.id]) 
                this.data.callback.callback = bird.Apps[this.id][d.callback];
...
{
    console.log('---------------running-------------------------'+this.id+':'+this.data.context, data);
    if(!('feed' in this.data && this.data.feed)) {
        return false;
    }
    var u = this.data.feed, p=[], firstRun=true;
    _feeds[u]=false;
...
function feedCheckScroll(e)
{
    ...
    var el=e.target;
    //console.log('feedCheckScroll: '+el.scrollHeight+' - '+el.scrollTop+' - '+el.offsetHeight+' < '+el.children[el.children.length-1].offsetHeight, el.scrollHeight - el.scrollTop - el.offsetHeight < el.children[el.children.length-1].offsetHeight);
