...
bird.Camera = function(d) 
{
    ...
    console.log('this is a camera!', this);
};
bird.stream = null;
var _cameraApp = null;
var _toUpload = [];
...
bird.Camera.prototype.loadEnd = function()
{
    ...
    if(_cameraEnabled && _cameraNative) {
        ...
        s += '<form name="form-'+this.id+'" id="form-'+this.id+'" action="#">'
            + '<span class="app-camera-thumb app-camera-disk"><input type="file" name="f[]" id="f" multiple="multiple" accept="image/*;capture=camera" onchange="return bird.Camera.fileSelect(this);" /><span class="app-camera-thumb-title">'+bird.t.cameraDisk+'</span></span>'
            + '</form>';
    }
...
function onPhotoSuccess(s, p) {
    ...
    console.log('onPhotoSuccess: ', d, s);
    var app = bird.a[_cameraApp];
    var o = Sizzle('#'+app.id+' .camera-output')[0], cn='app-camera-img', uo, u;
    p.id='f'+(new Date().getTime());
    if(this.id in bird.Apps && 'renderEntry' in bird.Apps[this.id]) _renderEntry = bird.Apps[this.id].renderEntry;
    fn = _renderEntry;
...
function photoUploadSuccess(r) {
    ...
    var so = ('getAttribute' in this)?(Sizzle('#'+app.id+' .camera-output > #'+this.getAttribute('id'))):(false);
    if(so && so.length>0) {
        se = o.insertBefore(se, so[0]);
        o.removeChild(so[0]);
        so[0]=null;
