...
bird.runApp=function()
{
    if(!bird.flying) {
        ...
        _rT = setTimeout(bird.runApp, 10*1000);
        return;
    } else if(_rT) {
        clearTimeout(_rT);
        _rT=0;
...
bird.fly = function(id, pid) 
{
    if(typeof(id)!='string' || !id) {
        if(_load.length>0) {
            console.log('apps to load!', _load, bird.a);
            id = _load.shift();
    ...
    if(id in bird.a) {
        ...
        if(_first) {
            ...
            if(window.location.hash) {
                _load = window.location.hash.replace(/^#\/?/, '').split('/');
            }            
            if(bird.onReady.length>0) {
                var i=0;
                while(i<bird.onReady.length) {
                    bird.onReady[i](bird);
                    i++;
                }
            }
...
{
    var app = bird.addApp(a);
    if(_load.length>0) {
        console.log('applist: ', _load);
        bird.fly();
    } else {
        app.fly();
    }
...
bird.downloadApp = function(d)
{
    ...
    if(!app) {
        ...
        app.innerHTML = s;
        document.getElementById(this.id).appendChild(app);
    } else {
...
bird.createOverlay = function(s, id, cn)
{
    var c, dom=false;
    if(id) {
        c=document.getElementById('bird-camera');
        if(c) dom=true;
    }
    if(!c) {
...
{
    bird.flying = true;
    var o=document.getElementsByClassName('app-overlay'), i=o.length, co, j=0;
    while(i-- > 0) {
        co = Sizzle('video',o[i]);
        j=co.length;
        while(j-- > 0) {
...
bird.App.prototype = {
    ...
    init: function(a) {
        ...
        if('toggleApps' in a) this.toggleApps = (a.toggleApps); 
        this.data={};
        if('apps' in a) {
    ...
    fly: function(pid)
    {
        ...
        if(this.credentials) {
            ...
            if(!auth) {
                ...
                console.log(this.id, 'needs auth', this.credentials, _user);
                _load.unshift(this.id);
                getUserInfo();
                return false;
            }
        }
    ...
    load: function()
    {
        ...
        var al=this.apps.length,i, s='<div class="app-container"></div>';
        if(al) {
            ...
            while(i<al){
                s += '<a class="app-thumb app-thumb-'+this.apps[i]+' thumb-'+bird.a[this.apps[i]].type+'" onclick="return bird.fly(\''+this.apps[i]+'\', \''+this.id+'\')">'
                   +   ((bird.a[this.apps[i]].label)?('<span class="app-thumb-title">'+bird.encodeHtml(bird.a[this.apps[i]].label)+'</span>'):(''))
                   + '</a>';
                i++;
            }
...
function getUserInfo(d)
{
    if(d) {
        ...
        var s = ('content' in _user)?(_user.content):('<p class="error">'+bird.t.error403+'</p>');
        c = bird.createOverlay(s, 'app-user-auth');
        _wc.push(refreshUserInfo);
    }
}
...
function makeMenu(slide)
{
    ...
    if(slide && slide.search(/^(top|left|right|bottom)$/)>-1) {
        _menuTarget[id]=[this.offsetLeft, this.offsetTop];
...
function getPointer(e) {
    ...
    if(e.target && id && id in _menuToggle && x > 0 && y > 0) {
        ...
        if((d=='left' || d=='right') && Math.abs(_mv[1]) <10 && Math.abs(_mv[0]) > prop.w*0.5) {
        ...
        } else if((d=='top' || d=='bottom') && Math.abs(_mv[0]) <10 && Math.abs(_mv[1]) > prop.h*0.6) {
...
function menuHover(e) {
    ...
    var f=_menu[id], i=f.length, c=null, end=false, el;
    if(_m[0]>=0 && _m[1]>=0) {
        var x = _m[0], y=_m[1];
        if(id in _menuTarget) {
            if(typeof(e)=='string') {
                ...
                x -= e.target.offsetLeft - _menuTarget[id][0];
                y -= e.target.offsetTop - _menuTarget[id][1];
            }
        }
        while(i-- > 0) {
        ...
        end = (c && typeof(e)!='string' && (e.type=='mouseup' || e.type=='touchend'));
    }
...
function checkAppStatus()
{
    ...
    if(bird.b.length>0) {
        ...
        if(!bird.keepHistory && bird.b.length>1) {
            if(window.getComputedStyle) {
