...
{
    //console.log('dispatchEvent '+ev+' on #'+a.id+', internalStatus: '+bird.a[a.id].internalStatus);
    var r=true;
...
var _again=3;
bird.tryAgain=function()
{
    bird.timeout('runApp', _again*1000);
    _again *= 0.5;
    if(_again > 180) _again=180;
}
...
bird.fly = function(id, pid) 
{
    ...
    if(id in bird.a) {
        ...
        if(pid) {
            bird.wget(((bird.r)?(bird.r):(''))+id+'.json', bird.startApp, bird.error, 'json', document.getElementById(pid));
        } else {
            bird.wget(((!first && bird.r)?(bird.r):(''))+id+'.json', bird.startApp, bird.error, 'json');
        }
    }
...
bird.land = function(id)
{
    ...
    if(document.getElementsByClassName('app-overlay').length>0) {
        ...
        if(bird.exit) bird.confirm(bird.t('ExitMessage'), bird.exit, bird.t('ExitTitle'), bird.t('ExitButtons'));
    } else {
...
bird.exit = function(){
    ...
    bird.addHtml(c, bird.t('Bye'));//+'<button onclick="bird.removeElement(document.getElementsByClassName(\'app-exit\')[0]);bird.fly(\''+id+'\');return false;">'+bird.t('Restart')+'</button>';
...
bird.downloadApp = function(d)
{
    ...
    var la=app.getElementsByTagName('a');
    i=la.length;
    while(i-- > 0) {
        a=la.item(i);
        if(a && !a.getAttribute('target') && a.getAttribute('href')) {
            bird.trigger(a, bird.inAppPreview);
        }
        a=null;
    }
    la=null;
    la=app.getElementsByTagName('script');
    i=-1;
    while(++i < la.length) {
        a=la.item(i);
        if(a) (new Function(a.innerHTML))();
        a=null;
    }
    la=null;
...
bird.removeElement=function(e)
{
    if(typeof(e)=='string' && e) {
        return bird.removeElement(document.getElementById(e));
    } else if(e) {
...
bird.createOverlay = function(s, id, cn)
{
    ...
    bird.addHtml(c, '<div class="app-overlay-container app-centered">'+s+'</div>');
    if(!update) {
    ...
    if(b.length==0) {
        ...
        bird.addHtml(b[0], bird.t('cancelButton'));
        b[0] = c.children[0].appendChild(b[0]);
        i=1;
    }
    while(i-- > 0) {
        if('item' in b) {
            bird.fastTrigger(b.item(i), bird.closeOverlay);
        } else {
            bird.fastTrigger(b[i], bird.closeOverlay);
            b[i]=null;
        }
    }
    b=null;
    ...
    while(i-- > 0) {
        if(!b[i].getAttribute('target') && b[i].getAttribute('href')) {
            bird.trigger(b[i], bird.inAppPreview);
        }
        b[i]=null;
    }
    //bird.flying = false;
...
bird.closeOverlay = function(e)
{
    ...
    if(i>0) {
        if(!removed) {
            while(i-- > 0) {
                bird.removeElement(o.item(i)); // IE bug when accesing as [i]
                removed = true;
                break;
            }
        }
...
function inAppClose()
{
    ...
    if(_w) _w.close();
    else if(window.onfocus) window.focus=null;
    _w = null;
    //bird.runApp();
    refreshUserInfo();
    bird.fly();
...
bird.App.prototype = {
    ...
    init: function(a) {
        ...
        if('uri' in a) {
            this.uri = a.uri;
            if(bird.r && this.uri.substr(0,1)=='/') this.uri=bird.r+this.uri;
        }
        ...
        if(bird.apps.length==0) { // main app
            ...
            //if('persistent' in a && a.persistent) bird.App.prototype.persistent = true;
        ...
        if('persistent' in a && a.persistent) this.persistent = true;
    ...
    load: function()
    {
        ...
        var app=document.getElementById(this.id),i=0,s='';
        if(this.id in _appFrags) {
            app.appendChild(_appFrags[this.id]);
            _appFrags[this.id]=null;
            delete(_appFrags[this.id]);
        }
        if(app.children.length==0) {
            var al=this.apps.length;
            s='<div class="app-container">'+((this.content)?(this.content):(''))+'</div>';
            if(al) {
                s+='<div class="apps app-thumbs">';
                i=0;
                while(i<al){
                    s += '<a class="app-thumb app-thumb-'+this.apps[i]+' thumb-'+bird.a[this.apps[i]].type+'"'
                       +   ' onclick="bird.fly(\''+this.apps[i]+'\', \''+this.id+'\');return false;"'
                       //+   ' ontouchstart="bird.fly(\''+this.apps[i]+'\', \''+this.id+'\');return false;"'
                       //+   ' onmousedown="bird.fly(\''+this.apps[i]+'\', \''+this.id+'\');return false;"'
                       +   '>'
                       +   ((bird.a[this.apps[i]].label)?('<span class="app-thumb-title">'+bird.a[this.apps[i]].label+'</span>'):(''))
                       + '</a>';
                    i++;
                }
                s+='<a class="app-thumb app-thumb-back"'
                  //+ ' ontouchstart="bird.land();return false;"'
                  //+ ' onmousedown="bird.land();return false;"'
                  + ' onclick="if(bird.noMenu(true))bird.land();return false;"'
                  + '></a>';
                s+='</div>';
            }
            al=null;
            i=null;
            if(bird.b.length>0) {
                s+='<a class="app-thumb app-close"'
                //+ ' ontouchstart="bird.noMenu();bird.land(\''+this.id+'\');return false;"'
                //+ ' onmousedown="bird.noMenu();bird.land(\''+this.id+'\');return false;"'
                + ' onclick="if(bird.noMenu(true))bird.land(\''+this.id+'\');return false;"'
                + '></a>';
            }
            bird.addHtml(app, s);
        }
    ...
    afterLoad: function()
    {
        ...
        if(this.apps.length>0) { //  && 'ontouchmove' in app
            var t=app.getElementsByClassName('app-thumbs').item(0); // IE bug when accesing as [0]
            if(this.menu) {
                //console.log('daqui?1', t);
    ...
    afterClose: function()
    {
        ...
        if(app) {
            if(this.persistent) {
                ...
                _appFrags[this.id]=document.createDocumentFragment();
                var i=0;
                while(app.children.length > 0) {
                    _appFrags[this.id].appendChild(app.children[0]);
                    i++;
                    if(i>10)break;
                }
            }
...
function getMenuItem(id, e)
{
    ...
    if(_m[0]>=0 && _m[1]>=0 && id in _menu) {
        var f=_menu[id], i=f.length, el=(typeof(e)=='string' || e.target.getAttribute('id')!=id)?(document.getElementById(id)):(e.target);
...
{
    //console.log('makeMenu on '+slide, this.getAttribute('id'));
    ...
    if(!(id in _menu)) {
        ...
        if (window.navigator.msPointerEnabled) {
            this.setAttribute('style','-ms-touch-action: none');
            this.addEventListener('MSPointerMove', menuHover, false);
            this.addEventListener('MSPointerUp', menuHover, false);
        }
    }
    if(slide && slide.search(/^(top|left|right|bottom)$/)>-1) {
        _menuToggle[id] = slide;
    }
    //console.log('making menu at '+id);
    _menu[id] = f;
...
bird.noMenu=function(set)
{
    var t=0;
    if(bird.noMenu.timeout) {
        t=new Date().getTime();
        if(t<bird.noMenu.timeout) return false;
    }
    if(set) {
        if(!t) t=new Date().getTime();
        bird.noMenu.timeout = t+400;
    }
    return true;
}
bird.noMenu.timeout=0;
...
function menuHover(e) {
    ...
    if((!id || !(id in _menu)) && ('getAttribute' in this)) {
        if(!id) id=this.getAttribute('id');
        if(!(id in _menu)) {
            var o=this;
            while(o.parentNode) {
                o=o.parentNode;
                if((id=o.getAttribute('id')) && id in _menu) break;
            }
        }
    }
    //console.log('menuHover: ', typeof e=='string' ? e : e.type,' '+id+' ', e, this );
    if(!id || !(id in _menu)) {
        return;
    }
    //console.log(' go on...', _menu[id]);
    ...
    if(!c || c.className.search(/\bhover\b/)<0) {
        ...
        while(i-- > 0){
            ro=r.item(i);
            ro.className = ro.className.replace(/\s*\bhover\b/, '');
            ro=null;
        }
...
function closeApp(e)
{
    ...
    for(i=os.length -1; i>=0; i--) {
        id=('item' in os)?(os.item(i).getAttribute('id')):(os[i].getAttribute('id'));
        if(id && id in bird.a) {
            bird.a[id].close();
        }
    }
        ...
        if(ar) {
            ...
            for(i=ar.length -1; i>=0; i--) {
                ...
                if(id && id in bird.a && cid!=id) {
                    cs = e.className.replace(/\s*\bapp-active\b/, ' app-inactive');
                    if(e.className!=cs) e.className=cs;
                }
                e=null;
            }
        }
...
function checkHealth()
{
    ...
    if(!bird.online || !bird.flying || bird.b.length==0 || _w) {
        //console.log('why: ', (!bird.online), (!bird.flying), bird.b.length==0, (_w));
        return false;
    }
    for(var n in bird.timeoutQueue) {
        //console.log('why: timeoutQueue');
        return false;
    }
    if(document.getElementsByClassName('app-overlay').length>0) {
        //console.log('why: overlay');
        return false;
    }
