...
bird.Feed.prototype.run = function(data)
{
    ...
    if(data) { //feed
                    ...
                    if(!n) {
                        ...
                        */
                        if(this.data.callback) {
                            bird.trigger(ref, this.data.callback);
                        }
        ...
        bird.wget(u, this.run, bird.tryAgain, 'json', this);
    }
}
...
bird.renderEntry = function(e)
{
    ...
    if(e.video && e.video.length > 0) {
        c.className += ' video';
        c.setAttribute('data-video', JSON.stringify(e.video));
    }
...
function feedCheckScroll(e)
{
    ...
    if(el.scrollHeight - el.scrollTop - el.offsetHeight < el.children[el.children.length-1].offsetHeight) {
        ...
        if(l && t) {
            ...
            bird.wget(u, app.run, bird.tryAgain, 'json', app);
